...
const InteractiveNeuralVortex = () => {
  ...
  useEffect(() => {
    ...
    const vertexShader = compileShader(vsSource, glCtx.VERTEX_SHADER);
    const fragmentShader = compileShader(fsSource, glCtx.FRAGMENT_SHADER);
    ...
    const uPointer = glCtx.getUniformLocation(program, 'u_pointer_position');
    const uScroll = glCtx.getUniformLocation(program, 'u_scroll_progress');
    ...
    const render = () => {
      ...
      glCtx.uniform1f(uTime, performance.now());
      glCtx.uniform2f(uPointer, pointer.current.x / window.innerWidth, 1 - pointer.current.y / window.innerHeight);
      glCtx.uniform1f(uScroll, window.pageYOffset / (2 * window.innerHeight));
